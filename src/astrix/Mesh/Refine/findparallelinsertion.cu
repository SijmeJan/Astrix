...
__host__ __device__
void LockTriangles(const real2 VcAdd,
		 ...
		 real *pParam, unsigned int *pRandom)
{
  ...
  while (i < nRefine) {
    LockTriangles(pVcAdd[i], pElementAdd[i], nTriangle, pTiC,
...
__host__ __device__
void FindIndependentCavities(int i, real2 *pVcAdd, int *pElementAdd,
			     int nTriangle, int *pTiC, int3 *pTv, int3 *pTe,
			   ...
			   unsigned int *pRandom, int *pUniqueFlag)
{
  ...
  while (i < nRefine) {
    FindIndependentCavities(i, pVcAdd, pElementAdd, nTriangle,
...
__host__ __device__
void FlagEdgesForChecking(int i, real2 *pVcAdd, int *pElementAdd,
			...
			const Predicates *pred, real *pParam, int *pEnC)
{
  ...
  while (i < nRefine) {
    FlagEdgesForChecking(i, pVcAdd, pElementAdd, nTriangle,
				      ...
				      const MeshParameter *meshParameter)
{
  ...
  if (cudaFlag == 1) {
    ...
    for (int n = 0; n < (int) nRefine; n++) 
      LockTriangles(pVcAdd[n], pElementAdd[n], nTriangle, pTiC,
		    pTv, pTe, pEt, pVc, nVertex, Px, Py, predicates,
  ...
  if (cudaFlag == 1) {
    ...
    for (int n = 0; n < (int) nRefine; n++) 
      FindIndependentCavities(n, pVcAdd, pElementAdd, nTriangle,
			      pTiC, pTv, pTe, pEt, pVc, nVertex, Px, Py,
  ...
  if (cudaFlag == 1) {
    ...
    for (int n = 0; n < (int) nRefine; n++) 
      FlagEdgesForChecking(n, pVcAdd, pElementAdd, nTriangle,
			   pTv, pTe, pEt, pVc, nVertex, Px, Py,
